...
def load_data_and_model():
    ...
    df = df[['연번', '지원분야', '제목', '게시글', '대표답변']].copy()
...
if st.button('답변 받기', type='primary', use_container_width=True):
    if not query.strip():
        ...
        st.session_state.feedback = None  # 새 질문 시 feedback 초기화

        with st.spinner('유사 상담사례 검색 중...'):
            st.session_state.retrieved = retrieve(query, df, sbert_model, corpus_embeddings)
        ...
        with st.spinner('AI 답변 생성 중...'):
            st.session_state.answer = generate_answer(query, st.session_state.retrieved, announcements)

# ── 결과 출력 (session_state 기반) ───────────────────────
if st.session_state.answer:

    with st.expander('🔍 유사 상담사례 검색 결과', expanded=False):
        for r in st.session_state.retrieved:
