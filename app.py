...
def get_bizinfo_announcements(keyword, num_rows=3):
    ...
    try:
        ...
        return [{
            '출처':     '[비즈니스지원단]',
            '공고명':   i.get('pblancNm', ''),
            '지원내용': i.get('bsnsSumryCn', '')[:200],
            '신청기간': f"{i.get('reqstBgnDe','')} ~ {i.get('reqstEndDe','')}",
            '담당기관': i.get('mngtMssofcNm', ''),
        } for i in item_list]
    except:
        return []
...
# ── 지원사업 질문 판단 ───────────────────────────────────
SUPPORT_KEYWORDS = ['지원금', '지원사업', '창업', '보조금', '융자',
                    '정책자금', '공고', '신청', '자금', '지원']
